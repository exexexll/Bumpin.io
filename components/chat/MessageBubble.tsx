...
export function MessageBubble({ message, isOwn, showSender = true }: MessageBubbleProps) {
  ...
  return (
    ...
    >
      ...
      {/* Message Content */}
      <div className={`flex flex-col ${isOwn ? 'items-end' : 'items-start'} w-full max-w-[85%] sm:max-w-[75%]`}>
        ...
        <div
          ...
          } ${isOwn ? 'rounded-tr-sm' : 'rounded-tl-sm'} max-w-full`}
        >
          {/* Text Message */}
          {message.messageType === 'text' && (
            <p className="text-sm whitespace-pre-wrap break-words overflow-wrap-anywhere word-break-break-word">{message.content}</p>
          )}
          ...
          {message.messageType === 'image' && message.fileUrl && (
            <div className="space-y-2">
              {message.content && (
                <p className="text-sm whitespace-pre-wrap break-words overflow-wrap-anywhere mb-2">{message.content}</p>
              )}
        ...
        <div className="flex items-center gap-1.5 mt-1 ml-1">
          ...
          {isOwn && message.readAt && (
            <svg className="w-3 h-3 text-[#ffc46a]" fill="currentColor" viewBox="0 0 20 20">
